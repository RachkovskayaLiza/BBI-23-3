    ...
    protected override void Solution()
    {
        ...
        for (int i = 0; i < text.Length; i++)
        {
            if (checker.Contains(text.ToUpper()[i]))
            {
                ...
                if (_tmpLength > 50)
                {
                    string[] l = _answer.Split(' ', StringSplitOptions.RemoveEmptyEntries);

                    for (int j = 0; j < 50 - _keepLength; j++)
                        _answer = _answer.Insert(_answer.Length - l[l.Length - 1].Length - 1, " ");
        ...
        string[] _l = _answer.Split(' ', StringSplitOptions.RemoveEmptyEntries);
